...
float DW1000RangingClass::lastRange;
// data buffer
			...
			if(_type == ANCHOR) {
				...
				if(messageType == POLL) {
                   ...
                   for(uint8_t i = 0; i < 2; i++) {
                        ...
                        byte adress[2];
                        memcpy(adress,data+SHORT_MAC_LEN+2+4*i, 2);
                        if((adress[0]!=_currentShortAddress[0])&&(adress[1]!=_currentShortAddress[1]))
                        {
                            memcpy(&lastRange,data+SHORT_MAC_LEN+30+i*5, 4);
...
void DW1000RangingClass::transmitPoll(DW1000Device* myDistantDevice) {
	...
	if(myDistantDevice == NULL) {
		...
		for(uint8_t i = 0; i < _networkDevicesNumber; i++) {
            ...
            float distance = _networkDevices[i].getRange();
			memcpy(data+SHORT_MAC_LEN+30+i*5,&distance, 4);
		}
...
void DW1000RangingClass::clearData()
{
